...
def run_pipeline():
    ...
    patients     = pd.read_csv(DATA_DIR + "patients.csv", on_bad_lines="skip", usecols=lambda c: c in ["Id","BIRTHDATE","DEATHDATE","GENDER","RACE","ETHNICITY","MARITAL","INCOME","HEALTHCARE_COVERAGE"])
    encounters   = pd.read_csv(DATA_DIR + "encounters.csv", on_bad_lines="skip", usecols=lambda c: c in ["PATIENT", "START", "Id","ENCOUNTERCLASS","BASE_ENCOUNTER_COST","TOTAL_CLAIM_COST","PAYER_COVERAGE"])
    observations = pd.read_csv(DATA_DIR + "observations.csv", on_bad_lines="skip", usecols=lambda c: c in ["PATIENT","DESCRIPTION","VALUE"])
    conditions   = pd.read_csv(DATA_DIR + "conditions.csv", on_bad_lines="skip", dayfirst=True, usecols=lambda c: c in ["PATIENT", "START", "DESCRIPTION"])
    medications  = pd.read_csv(DATA_DIR + "medications.csv", on_bad_lines="skip", usecols=lambda c: c in ["PATIENT", "START", "DESCRIPTION", "BASE_COST", "DISPENSES"])
    procedures   = pd.read_csv(DATA_DIR + "procedures.csv", on_bad_lines="skip", usecols=lambda c: c in ["PATIENT", "START", "DESCRIPTION", "BASE_COST"])
    immunizations= pd.read_csv(DATA_DIR + "immunizations.csv", on_bad_lines="skip", usecols=lambda c: c in ["PATIENT", "DATE", "DESCRIPTION"])
    allergies    = pd.read_csv(DATA_DIR + "allergies.csv", on_bad_lines="skip", usecols=lambda c: c in ["PATIENT", "START", "TYPE", "CATEGORY"])
    careplans    = pd.read_csv(DATA_DIR + "careplans.csv", on_bad_lines="skip", usecols=lambda c: c in ["PATIENT", "Id", "REASONDESCRIPTION"])
    imaging      = pd.read_csv(DATA_DIR + "imaging_studies.csv", on_bad_lines="skip", usecols=lambda c: c in ["PATIENT", "Id", "MODALITY_DESCRIPTION", "BODYSITE_DESCRIPTION"])
    devices      = pd.read_csv(DATA_DIR + "devices.csv", on_bad_lines="skip", usecols=lambda c: c in ["PATIENT", "START", "DESCRIPTION"])
    supplies     = pd.read_csv(DATA_DIR + "supplies.csv", on_bad_lines="skip", usecols=lambda c: c in ["PATIENT", "DATE", "DESCRIPTION"])
    payer_trans  = pd.read_csv(DATA_DIR + "payer_transitions.csv", on_bad_lines="skip", usecols=lambda c: c in ["PATIENT", "START_DATE", "PAYER"])
    claims       = pd.read_csv(DATA_DIR + "claims.csv", on_bad_lines="skip", usecols=lambda c: c in ["PATIENTID", "Id", "OUTSTANDING1", "CURRENT1", "TOTAL_CLAIM_COST"])
